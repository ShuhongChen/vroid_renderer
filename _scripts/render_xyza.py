...
# name,bn = ap.parse_args().name.split(',')
...
from _databacks import lustrous_v1 as ulust
...
# elevations = [e.item() for e,a in ulust.cam60]
# azimuths = [a.item() for e,a in ulust.cam60]
# distances = [1.0,] * 60
# fovs = [30.0,] * 60
# n_generations = len(elevations)
    ...
    def __init__(self, program=None, loc_origin=None, **kwargs):
        ...
        self.program = mglw.resources.programs.load(
            mglw.meta.ProgramDescription(
                ...
                path=Path("./_cells/shaders/rutileE_xyza.glsl").resolve(),
            )
        )
...
def main():
    ...
    for i,out in enumerate(outs):
        ...
        out.save(mkfile(f'{odn}/{render_dtype}/{franch}/{bn}/{i:04d}.png'))
        # out.resize(mini_size, mini_mode).bg(mini_bg).save(mkfile(f'{odn}/{render_dtype}/{franch}/{bn}/{i:04d}.png'))
        # out.save(mkfile(f'{odn}/{render_dtype}/{franch}/{bn}/{i:04d}.png'))
